...
def add_event(request):
    if "email" in request.session:
        ...
        if request.POST:
            ...
            if "pic" in request.FILES:
                eid = Event.objects.create(user_id = uid, title = title, description = description , pic = pic)

            else:
                eid = Event.objects.create(user_id = uid, title = title, description = description)
...
#                     'nid' : nid,
#         }
#         return render(request,"myapp/edit_notice.html",contex)
